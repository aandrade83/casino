//Set game vars
var core_url = 'https://play.casinogamesonline.com/utilities/games/video_poker_deuces/action.php?gid='+gid+'&';
...
var is_dealed = false;
var coins = 0;
var coin_light_positions = new Array();
var prize_light_positions = new Array();
var is_mute = false;
var set_mute = false;
...
function preload (){
	...
	//Load elements
	//Images
	this.load.image('background', '/utilities/games/video_poker_deuces/imgs/back.jpg?v3');
	this.load.image('card_back', '/utilities/games/video_poker_deuces/imgs/card_back.png');
	this.load.image('hold_off', '/utilities/games/video_poker_deuces/imgs/hold_off.png');
	this.load.image('hold_on', '/utilities/games/video_poker_deuces/imgs/hold_on.png?v5');
	this.load.image('coin_light', '/utilities/games/video_poker_deuces/imgs/coin_light.png?v2');
	this.load.image('prize_light', '/utilities/games/video_poker_deuces/imgs/prize_light.png?v2');
	for(var i = 0; i < deck.length; i++){
		this.load.image('card_'+deck[i], '/utilities/images/games/cards/'+deck[i]+'.png');
	}
	//Sounds
	this.load.audio('background_sound', '/utilities/games/video_poker_deuces/sounds/sound_back.mp3');
	this.load.audio('winning_sound', '/utilities/games/video_poker_deuces/sounds/win6.mp3');
	this.load.audio('card_sound', '/utilities/games/video_poker_deuces/sounds/card.mp3');
...
function create (){
	...
	//lights
	coin_light = this.add.image(game_width/2.16, game_height/6.4, 'coin_light');
	coin_light.displayWidth=game.config.width*0.044; 
	coin_light.scaleY=coin_light.scaleX;
	coin_light.alpha = 0.4;
	
	coin_light_positions.push(game_width/2.16);
	coin_light_positions.push(game_width/1.975);
	coin_light_positions.push(game_width/1.815);
	coin_light_positions.push(game_width/1.68);
	coin_light_positions.push(game_width/1.56);
	
	prize_light = this.add.image(game_width/2.01, game_height+500, 'prize_light');
	prize_light.displayWidth=game.config.width*0.33; 
	prize_light.scaleY=prize_light.scaleX;
	prize_light.alpha = 0.4;
	
	prize_light_positions.push(game_height+500);
	prize_light_positions.push(game_height/3.37);
	prize_light_positions.push(game_height/3.75);
	prize_light_positions.push(game_height/4.25);
	prize_light_positions.push(game_height/4.92);
	prize_light_positions.push(game_height/5.77);
	prize_light_positions.push(game_height/7.17);
	prize_light_positions.push(game_height/9.21);
	prize_light_positions.push(game_height/12.71);
	prize_light_positions.push(game_height/21.8);
	prize_light_positions.push(game_height/71.2);
	
	
	//cards
	cards[0] = this.add.image(game_width/2.83, game_height/2.3, 'card_back');
	cards[0].displayWidth=game.config.width*0.069; 
	cards[0].scaleY=cards[0].scaleX;
	
	cards[1] = this.add.image(game_width/2.355, game_height/2.3, 'card_back');
	cards[1].displayWidth=game.config.width*0.069; 
	cards[1].scaleY=cards[1].scaleX;
	
	cards[2] = this.add.image(game_width/2.015, game_height/2.3, 'card_back');
	cards[2].displayWidth=game.config.width*0.069; 
	cards[2].scaleY=cards[2].scaleX;
	
	cards[3] = this.add.image(game_width/1.76, game_height/2.3, 'card_back');
	cards[3].displayWidth=game.config.width*0.069; 
	cards[3].scaleY=cards[3].scaleX;
	
	cards[4] = this.add.image(game_width/1.562, game_height/2.3, 'card_back');
	cards[4].displayWidth=game.config.width*0.069; 
	cards[4].scaleY=cards[4].scaleX;
	
	//Hold BTNS
	hold_btns[0] = this.add.image(game_width/2.83, game_height/1.75, 'hold_off').setInteractive();
	hold_btns[0].displayWidth=game.config.width*0.052; 
	hold_btns[0].scaleY=hold_btns[0].scaleX;	
	hold_btns[0].alpha = 0.4;
	hold_btns[0].on('pointerup', function (pointer) {hold_card(0);});	
	
	hold_btns[1] = this.add.image(game_width/2.355, game_height/1.75, 'hold_off').setInteractive();
	hold_btns[1].displayWidth=game.config.width*0.052; 
	hold_btns[1].scaleY=hold_btns[1].scaleX;
	hold_btns[1].alpha = 0.4;
	hold_btns[1].on('pointerup', function (pointer) {hold_card(1);});	
	
	hold_btns[2] = this.add.image(game_width/2.015, game_height/1.75, 'hold_off').setInteractive();
	hold_btns[2].displayWidth=game.config.width*0.052; 
	hold_btns[2].scaleY=hold_btns[2].scaleX;
	hold_btns[2].alpha = 0.4;
	hold_btns[2].on('pointerup', function (pointer) {hold_card(2);});	
	
	hold_btns[3] = this.add.image(game_width/1.76, game_height/1.75, 'hold_off').setInteractive();
	hold_btns[3].displayWidth=game.config.width*0.052; 
	hold_btns[3].scaleY=hold_btns[3].scaleX;
	hold_btns[3].alpha = 0.4;
	hold_btns[3].on('pointerup', function (pointer) {hold_card(3);});	
	
	hold_btns[4] = this.add.image(game_width/1.562, game_height/1.75, 'hold_off').setInteractive();
	hold_btns[4].displayWidth=game.config.width*0.052; 
	hold_btns[4].scaleY=hold_btns[4].scaleX;
	hold_btns[4].alpha = 0.4;
	...
	test_text = this.add.text(game_width/4, game_height/1.341, "X: Y:", machine_text_style);
	test_text.alpha = 0;
	...
	$("#betone_max").css("margin-left",(game_width*0.535)+"px");
	$("#deal_btn").css("margin-left",(game_width*0.615)+"px");
...
function move_coin_light(){
	coin_light.x = coin_light_positions[coins-1];	
}
...
function highlight_prize(level){
	prize_light.y = prize_light_positions[level];
	var timer = 300;
	for(var i = 0; i < 5; i++){
		setTimeout("prize_light.alpha = 0;",timer);
		setTimeout("prize_light.alpha = 0.4;",timer+300);
		timer += 600;
	}
}
...
function deal(){
	if(!is_dealed){
		if(total_bet <= current_balance){
			...
			$.getJSON(core_url + "action=deal&bet=" + total_bet + "&coins=" + coins + "&pnr="+player_number ,function(data){
				
				if(!data.error){ //change the way to check error
				
					win_amount_text.text = "0";
					update_balance(current_balance-total_bet);		
					prize_light.y = prize_light_positions[0];
		...
		$.getJSON(core_url + "action=draw&holds=" + holds ,function(data){
			...
			if(!data.error){ //change the way to check error		
				...
				highlight_prize(data.win_level);
