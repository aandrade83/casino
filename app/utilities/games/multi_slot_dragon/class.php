...
class multi_slot{
	...
	function __construct($id_player) {
		 ...
		 $this->figures_on_reels[1] = array(
								  ...
								  array("figure"=>"fig8","amount"=>"20"),
								  array("figure"=>"fig9","amount"=>"22"),
								  array("figure"=>"fig10","amount"=>"24"),
								  array("figure"=>"fig11","amount"=>"26"),
								  array("figure"=>"fig12","amount"=>"28"),
								  array("figure"=>"fig13","amount"=>"30")
							 );
	...
	function get_prize($bet_amount, $line_position ,$reels_result){
		...
		if($reels_result[$line[0]-1] == $reels_result[$line[1]-1] && $reels_result[$line[1]-1] == $reels_result[$line[2]-1] && $reels_result[$line[2]-1] == $reels_result[$line[3]-1] && $reels_result[$line[3]-1] == $reels_result[$line[4]-1]){			
			...
			switch($reels_result[$line[0]-1]){ 
				...
				case "fig11": $win = $bet_amount * 150; $fsp = 0; break;
				case "fig12": $win = $bet_amount * 100; $fsp = 0; break;
				case "fig13": $win = $bet_amount * 75; $fsp = 5;  break;
			}
			...
			switch($reels_result[$line[0]-1]){ 
				...
				case "fig11": $win = $bet_amount * 75; $fsp = 0; break;
				case "fig12": $win = $bet_amount * 50; $fsp = 0;  break;
				case "fig13": $win = $bet_amount * 25; $fsp = 0;  break;
			}
			...
			switch($reels_result[$line[0]-1]){ 
				...
				case "fig9": $win = $bet_amount * 120;  break;
				case "fig10": $win = $bet_amount * 90;  break;
				case "fig11": $win = $bet_amount * 50; $fsp = 0; break;
				case "fig12": $win = $bet_amount * 30; $fsp = 0; break;
				case "fig13": $win = $bet_amount * 15; $fsp = 0;   break;
			}
	...
	function generate_reel_result($reel_content){
		...
		$fig1 = $reel_content[mt_rand(0,213)];
		$fig2 = $fig1;
		while($fig2 == $fig1){
			$fig2 = $reel_content[mt_rand(0,213)];
		}
		$fig3 = $fig1;
		while($fig3 == $fig1 || $fig3 == $fig2){
			$fig3 = $reel_content[mt_rand(0,213)];
		}
	...
	function set_reels_content($fixed = true){
		...
		if($fixed){
			...
			$json = file_get_contents($_SERVER['DOCUMENT_ROOT']."/utilities/games/multi_slot_dragon/json/dragon.txt"); 		
