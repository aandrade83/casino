...
if($_logged){
	...
	if(!is_null($game) && $game ->vars["path"] == "multi_slot_dragon"){
		...
		switch($action){
			...
			case "spin":
				...
				foreach($player_nums as $pnum){
					if(!is_numeric($pnum) || $pnum > 213 || $player_num < 0){$pnum = 0;}
				...
				if(is_numeric($bet_amount)){
					...
					if($_player->in_between_limits($bet_amount)){
						...
						if($bet_amount <= $current_balance || !is_null($free_spin)){
							...
							if($bet_amount >= $game ->vars["min_amount"] && $bet_amount <= $game ->vars["max_amount"]*25){
								...
								if(!is_null($free_spin)){
									...
									$bal_res = wallet_balance($_player->vars["id"]);
									$fs_balance = $bal_res["success"] ? floatval($bal_res["data"]["real"]) : 0;
									$bet_success = true;
								}else{
									//use money
									$bet_res = wallet_bet($_player->vars["id"], $bet_amount, "bet", $game->vars["id"]);
									$fs_balance = 0;
									$bet_success = $bet_res["success"];
								}
								...
								if($bet_success){
									...
									if($_company ->vars["prov_fair"]){
										//provalby fair
										$seed_data = discover_server_seed("drgns");
										$pf_result = sum_multi_nums($seed_data["xnr"],$player_num,",",213);
										...
										$hard_numbers = array();
										for($i=0;$i<15;$i++){
											$hard_numbers[] = mt_rand(0,213);
										}

										$next_seed = generate_server_seed("drgns",0,0,implode(",",$hard_numbers));
