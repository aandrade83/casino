...
function preload (){
	...
	//Load elements
	//Background
	this.load.image('background', 'utilities/games/blackjack_double/imgs/back.jpg');
	//chips
	...
	//Others
	this.load.image('arrow', 'utilities/games/blackjack_double/imgs/green_arrow.png');
	//sounds
	this.load.audio('card_flip_fx', 'utilities/games/blackjack_double/sounds/cardflip.wav');
		...
		if(!data.error){
			if(data.has_started_game){
				...
				if(data.splited*1){
					...
					var temp_dealer_cards = data.dealer_full_hand.split(",");
					
					setTimeout('deal_card("card_'+temp_player_cards2[0]+'",player_cards_positions[0].x,player_cards_positions[0].y);',500);
					setTimeout('deal_card("card_'+temp_dealer_cards[0]+'",dealer_cards_positions[0].x,dealer_cards_positions[0].y);',1000); //simulate flip to place card in same place as flipped ones
					setTimeout('deal_card("card_'+temp_player_cards[0]+'",player2_cards_positions[0].x,player2_cards_positions[0].y);',1500);
					setTimeout('deal_card("card_'+temp_dealer_cards[1]+'",dealer_cards_positions[1].x,dealer_cards_positions[1].y);',2000);
					...
					setTimeout('dealer_hand_text.text = "'+data.dealer_hand_value+'";',timer+500);	
	...
	if(game_status == "place_bets"){
		...
		$.getJSON(core_url + "action=deal&bet=" + current_bet  + "&pnr="+player_number,function(data){
			if(!data.error){ //change the way to check error 
				...
				setTimeout('deal_card("card_'+temp_dealer_cards[0]+'",dealer_cards_positions[0].x,dealer_cards_positions[0].y);',500); //simulate flip to place card in same place as flipped ones
				...
				setTimeout('deal_card("card_'+temp_dealer_cards[1]+'",dealer_cards_positions[1].x,dealer_cards_positions[1].y);',1500);
