...
if($_logged){
	...
	$action = param("action");
	$logic = new fortune_wheel($_player ->vars["id"]);
	$game = $_player->get_allowed_game($game_id);
	
	if(!is_null($game) && $game ->vars["path"] == "wheel_of_fortune"){
		...
		switch($action){ 
			...
			case "spin":
				...
				foreach($player_nums as $pnum){
					if(!is_numeric($pnum) || $pnum > 199 || $player_num < 0){$pnum = 0;} //change 109 in case of amount of figures in the reel change
							...
							if($bet_amount >= $game ->vars["min_amount"] && $bet_amount <= $game ->vars["max_amount"]){
								...
								if($_api->done){
									...
									if($_company ->vars["prov_fair"]){	
										...
										//remove this part when activated
