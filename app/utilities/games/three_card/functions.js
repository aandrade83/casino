...
function preload (){
	...
	//Load elements
	//Images
	this.load.image('background', 'utilities/games/three_card/imgs/back.jpg?v1');
	this.load.image('pay_table', 'utilities/games/three_card/imgs/pay_table.png');
	this.load.image('info', 'utilities/games/three_card/imgs/info.png');
	...
	this.load.image('area', 'utilities/games/three_card/imgs/blank_area.png');
	...
	//Sounds
	this.load.audio('card_flip_fx', 'utilities/games/three_card/sounds/cardflip.wav');
...
function create (){
	...
	//bet areas
	bet_areas_data.push({id: "ante", xpos: game_width/2.52, ypos: game_height/1.32, w: game.config.width*0.06, h:game.config.width*0.050});
	bet_areas_data.push({id: "pplus", xpos: game_width/2.02, ypos: game_height/1.32, w: game.config.width*0.06, h:game.config.width*0.050});
	bet_areas_data.push({id: "flop", xpos: game_width/1.69, ypos: game_height/1.32, w: game.config.width*0.06, h:game.config.width*0.050});
	...
	for(var i = 0; i < bet_areas_data.length; i++){
		...
		if(bet_areas_data[i].id != "flop"){bet_areas[bet_areas_data[i].id].on('pointerup', function (pointer) { place_bet(this.name); });}
	}
	
	clear_betareas_amounts();
	
	//card positions P1, P2, P3, P5, P5, D1, D2, D3, D4, D5
	cards_positions.push({x:game_width/2.5, y:game_height/1.85});
	cards_positions.push({x:game_width/2.33, y:game_height/1.85});
	cards_positions.push({x:game_width/2.18, y:game_height/1.85});	
	cards_positions.push({x:game_width/2.04, y:game_height/1.85});		
	cards_positions.push({x:game_width/1.92, y:game_height/1.85});
	
	cards_positions.push({x:game_width/2.5, y:game_height/3.62});
	cards_positions.push({x:game_width/2.33, y:game_height/3.62});
	cards_positions.push({x:game_width/2.18, y:game_height/3.62});	
	cards_positions.push({x:game_width/2.04, y:game_height/3.62});		
	cards_positions.push({x:game_width/1.92, y:game_height/3.62});
	
	//set text settings
	machine_text_style = { font: (game_width*0.01) + "px Arial", fill: "#ffffff", align: "center" };
	
	player_cards_text = this.add.text(game_width/1.98, game_height/1.85, "", machine_text_style);
	dealer_cards_text = this.add.text(game_width/1.98, game_height/3.62, "", machine_text_style);
	...
	test_text = this.add.text(game_width/30.95, game_height/2.03, "X: Y:", machine_text_style);
	test_text.alpha = 0;
	
	
	info_btn = this.add.image(game_width/1.06, game_height/1.1, 'info').setInteractive();
	...
	pay_table = this.add.image(game_width/5.2, game_height/2, 'pay_table').setInteractive();
	pay_table.displayWidth=game.config.width*0.20; 
	...
	game_levels_str.push("STRAIGHT FLUSH");//ROYAL
	game_levels_str[-1] = "DEALER DOES NOT QUALIFY";
...
function update (){
	...
	//clear PP chips from table
	var mov_pp_chips = 0;
	if(clear_pp_chips_to != ""){
		if(clear_pp_chips_to == "player"){
			for(var i=0; i<placed_chips.length; i++){
				if(placed_chips[i].area == "pplus"){
					mov_pp_chips ++;
					placed_chips[i].chip.y += game_width*0.01;
					if(placed_chips[i].chip.y > game_height + (placed_chips[i].chip.displayHeight)*2){
						placed_chips[i].chip.destroy();
			...
			if(mov_pp_chips == 0){clear_pp_chips_to = "";}
		}else if(clear_pp_chips_to == "dealer"){
			for(var i=0; i<placed_chips.length; i++){
				if(placed_chips[i].area == "pplus"){
					mov_pp_chips ++;
					placed_chips[i].chip.y -= game_width*0.01;
...
function place_bet(area){
	...
	if((!playing || area == "flop") && (open_bet_area == area || (open_bet_area == "ante" && area == "pplus") || loading_game)){
		...
		//if(/*current_bet+*/selected_chip <= current_balance){//not include current bet because current balance already have current bet deducted
			if(area_bets_amount[area]+selected_chip <= max_amount || (area_bets_amount[area]+selected_chip <= max_amount && area == 'flop')){
			
				if((area == "ante" || area == "pplus") && !loading_game){$("#btn_clear").slideDown(500);}
				if(current_bet+selected_chip >= min_amount && (area == "ante" || area == "pplus") && !loading_game){$("#btn_spin").slideDown(500);}
	...
	$.getJSON(core_url + "action=start",function(data){
		if(!data.error){
			if(data.has_started_game){
				...
				var player_cards = data.player_cards.split(","); 
				var dealer_card = data.dealer_card;
				setTimeout("deal_card(0,'card_"+player_cards[0]+"');",1);
				setTimeout("deal_card(1,'card_"+player_cards[1]+"');",200);
				setTimeout("deal_card(2,'card_"+player_cards[2]+"');",400);
				
				setTimeout("deal_card(5,'card_back');",600);
				setTimeout("deal_card(6,'card_back');",800);
				setTimeout("deal_card(7,'card_back');",1000);
				setTimeout("player_cards_text.text = '"+game_levels_str[data.player_level]+"';", 1500);
...
function place_rebet(deal){
	...
	var pre_chips = get_amount_chips_list(open_pp_bet);
	for(var e = 0; e<pre_chips.length; e++){
		change_chip(pre_chips[e]);
		place_bet("pplus");
	}
...
function deal(){
	...
	if(!is_dealed){
		if(current_bet <= current_balance){
			...
			$.getJSON(core_url + "action=deal&bet=" + area_bets_amount["ante"] + "&pplus=" + area_bets_amount["pplus"] + "&pnr="+player_number ,function(data){
				...
				if(!data.error){ //change the way to check error
					...
					is_dealed = true;
					game_status = data.game_status;
					open_bet = area_bets_amount["ante"];
					open_pp_bet = area_bets_amount["pplus"];
					open_bet_area = "flop";
					...
					setTimeout("deal_card(6,'card_back');",800);
					setTimeout("deal_card(7,'card_back');",1000);
					setTimeout("player_cards_text.text = '"+game_levels_str[data.player_level]+"';", 2000);
					
					if(data.pp_status != ""){
						setTimeout('complete_pp_bet("'+data.pp_status+'", '+data.pp_win_amount+', "'+data.game_status+'")',2500);
						setTimeout('update_balance('+data.balance+')',3000);
					}else{
						update_balance(data.balance);
						setTimeout('$("#btn_call").show(500); $("#btn_fold").show(500);',2200);
					}
...
function complete_pp_bet(status, win_amount, game_status){
	var timer = 1;
	if(status == "win"){
		//clear_pp_chips_to = "player"; //Keep chips on table, cleared to player at the end of the hand
		...
		area_bets_amount_text["pplus"].text = "";
		msg_txt = "You Lose";
		clear_pp_chips_to = "dealer";
	}
	
	if(game_status == "dealed"){
		setTimeout('$("#btn_call").show(500); $("#btn_fold").show(500);',timer+1000);
	}else{	
		setTimeout('preend_game("'+msg_txt+'")',timer+1000);
	}
	
}

function preend_game(msg){
	update_pf_data();
	show_message(msg);
		...
		if((open_bet) <= current_balance){
			...
			$.getJSON(core_url + "action=call" ,function(data){
				...
				if(!data.error){ //change the way to check error
					...
					setTimeout('show_dealer_cards("'+data.dealer_cards+'");',500);
					setTimeout("dealer_cards_text.text = '"+game_levels_str[data.dealer_level]+"';",1000);
					
					setTimeout('after_play('+data.win_amount+', "'+data.balance+'", "'+data.game_result+'")',1500);
...
function after_play(win_amount, balance, result){
	...
	if(result == 'win'){
		...
		winning_amount += win_amount;
		setTimeout("winned_chips_text.text = '"+currency_symbol+(winning_amount)+"';",timer+500);
	}else if(result == 'lose'){
		msg_txt = "You Lose";	
		clear_bets("dealer");
	}else if(result == 'push'){
