...
if($_logged){
	...
	if(!is_null($game) && $game ->vars["path"] == "multi_slot_FS"){
		...
		switch($action){
			...
			case "spin":
				...
				foreach($player_nums as $pnum){
					if(!is_numeric($pnum) || $pnum > 119 || $player_num < 0){$pnum = 0;}
							...
							if($bet_amount >= $game ->vars["min_amount"] && $bet_amount <= $game ->vars["max_amount"]*25){
								...
								if($bet_success){
									...
									if($_company ->vars["prov_fair"]){
										...
										$pf_result = sum_multi_nums($seed_data["xnr"],$player_num,",",119);
