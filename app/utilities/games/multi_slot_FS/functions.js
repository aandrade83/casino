//Set game vars
var core_url = '/utilities/games/multi_slot_FS/action.php?gid='+gid+'&';
...
function create (){
	...
	reels[1] = this.add.image(game_width/3.86, (game_height/-50)*52.5, 'reel2');
	reels[1].displayWidth=game.config.width*0.12; 
	reels[1].scaleY=reels[1].scaleX;
	reels[1].alpha = 0;
	
	reels[2] = this.add.image(game_width/2.29, (game_height/-50)*52.5, 'reel3');
	reels[2].displayWidth=game.config.width*0.12; 
	reels[2].scaleY=reels[2].scaleX;
	reels[2].alpha = 0;
	
	reels[3] = this.add.image(game_width/1.62, (game_height/-50)*52.5, 'reel4');
	reels[3].displayWidth=game.config.width*0.12; 
	reels[3].scaleY=reels[3].scaleX;
	reels[3].alpha = 0;
	
	reels[4] = this.add.image(game_width/1.25, (game_height/-50)*52.5, 'reel5');
	reels[4].displayWidth=game.config.width*0.12; 
	reels[4].scaleY=reels[4].scaleX;
	reels[4].alpha = 0;
	
	var yfposes = [6.36,2.46,1.53];
	var xfposes = [13,3.86,2.3,1.62,1.25];
	
	var fi = 0;
	var fignames = new Array("car","cash","horses","girl","bookie","cigar","hockey","basket","baseball","football");
	for(var i=0; i<xfposes.length; i++){
		for(var n=0; n<yfposes.length; n++){
			figures[fi] = this.add.image(game_width/xfposes[i], game_height/yfposes[n], "figure_" + fignames[ Math.floor(Math.random() * 10) ]);
	...
	//set texts positions
	bet_amount_text = this.add.text(game_width/10.5, game_height/1.15, current_bet, machine_text_style);
	lines_amount_text = this.add.text(game_width/3.9, game_height/1.15, 25, machine_text_style);
	balance_amount_text = this.add.text(game_width/1.105, game_height/6.13, current_balance, machine_text_style);
	freespins_amount_text = this.add.text(game_width/1.105, game_height/3.16, free_spins, machine_text_style);
	
	var initial_bet = current_bet * max_lines;
	if(free_spins > 0){
		initial_bet = "FREE";
	}	
	total_amount_text = this.add.text(game_width/1.105, game_height/2.15, initial_bet, machine_text_style);
	win_amount_text = this.add.text(game_width/1.105, game_height/1.64, "0", machine_text_style);
	
	var logo = this.add.image(game_width/1.91, game_height/1.18, 'logo');
	logo.displayWidth=game.config.width*0.23; 
	logo.scaleY=logo.scaleX;
	
	info_btn = this.add.image(game_width/1.06, game_height/1.36, 'info').setInteractive();
	...
	win_animation = this.add.sprite(game_width/2, game_height/2, 'animation_1');
...
function load_win_line(num,amount,sound){
	...
	if(!win_x5){winning_sounds[sound].play();}
}
...
function update_pf_data(){
	...
	if(pfdata){
		...
		for(var i = 0; i < 15; i++){
			new_pnum.push(getRandomInt(0,119));
		}
