...
function preload (){
	...
	//Load elements
	//Images
	this.load.image('background', 'utilities/games/caribbean/imgs/back.jpg?v1');
	this.load.image('pay_table', 'utilities/games/caribbean/imgs/pay_table.png');
	this.load.image('info', 'utilities/games/caribbean/imgs/info.png');
	...
	this.load.image('area', 'utilities/games/caribbean/imgs/blank_area.png');
	...
	//Sounds
	this.load.audio('card_flip_fx', 'utilities/games/caribbean/sounds/cardflip.wav');
...
function create (){
	...
	player_cards_text = this.add.text(game_width/1.74, game_height/1.85, "", machine_text_style);
	dealer_cards_text = this.add.text(game_width/1.74, game_height/3.62, "", machine_text_style);
	...
	pay_table.displayWidth=game.config.width*0.40; 
...
function place_bet(area){
	...
	if((!playing || area == "flop") && (open_bet_area == area || loading_game)){
		...
		if(/*current_bet+*/selected_chip <= current_balance){//not include current bet because current balance already have current bet deducted
			if(area_bets_amount[area]+selected_chip <= max_amount || (area_bets_amount[area]+selected_chip <= (max_amount*2) && area == 'flop')){
	...
	$.getJSON(core_url + "action=start",function(data){
		if(!data.error){
			if(data.has_started_game){
				...
				setTimeout("deal_card(3,'card_"+player_cards[3]+"');",600);
				setTimeout("deal_card(4,'card_"+player_cards[4]+"');",800);
				
				setTimeout("deal_card(5,'card_back');",1000);
				setTimeout("deal_card(6,'card_back');",1200);
				setTimeout("deal_card(7,'card_back');",1400);
				setTimeout("deal_card(8,'card_back');",1600);
				setTimeout("deal_card(9,'card_"+dealer_card+"');",1800);
