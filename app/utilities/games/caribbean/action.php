...
if($_logged){
	...
	$action = param("action");
	$logic = new _caribbean_poker($_player ->vars["id"]);
	$game = $_player->get_allowed_game($game_id);

	if(!is_null($game) && $game ->vars["path"] == "caribbean"){
				...
				if(is_numeric($bet_amount)){
					...
					if(is_null($session)){
						...
						if($_player->in_between_limits($bet_amount)){
							...
							if(($bet_amount*3) <= $current_balance){// checks for balance for ante and bet

								if($bet_amount >= ($game ->vars["min_amount"]/2) && $bet_amount <= ($game ->vars["max_amount"]/2)){ //checks ante
									...
									if($bet_res["success"]){
										...
										if($_company ->vars["prov_fair"]){
											$seed_data = discover_server_seed("cbbdpkr");
								...
								if($logic ->session ->vars["status"] == "dealed"){
									...
									if($bet_res["success"]){
										...
										if($logic ->using_pf){
											...
											$next_seed = generate_server_seed("cbbdpkr",0,100000000);
