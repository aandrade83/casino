...
if($_logged){
	...
	$action = param("action");
	$logic = new blackjack_sp($_player ->vars["id"]);
	$game = $_player->get_allowed_game($game_id);
	
	if(!is_null($game) && $game ->vars["path"] == "spanish_blackjack_contest"){
	
	switch($action){ 
		case "start":
			//start game, check if there is an started game and load it
			$session = get_blackjack_session_by_player($_player ->vars["id"], true, "spanish");
			if(!is_null($session)){
		...
		case "game_data":
			$session = get_blackjack_session_by_player($_player ->vars["id"],false, "spanish");
			if(!is_null($session)){
				$logic->load_data($session);			
				...
				if($session->vars["finished"]){
					...
					if($_company ->vars["prov_fair"]){ 
						...
						$next_seed = generate_server_seed("sbljk",0,100000000);										
						...
						if($bet_amount <= $balance["amount"]){
							...
							if($bet_amount >= $game ->vars["min_amount"] && $bet_amount <= $game ->vars["max_amount"]){
								...
								if($_api->done){
									...
									//provably fair
									if($_company ->vars["prov_fair"]){
										$seed_data = discover_server_seed("sbljk");
