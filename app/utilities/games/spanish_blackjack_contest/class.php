...
class blackjack_sp{
	...
	//SETTINGS
	var $number_of_decks = 6;
	
	function __construct($id_player) {
	   global $_deck;
	   
	   //initialize cards
	   $this ->cards = prepare_spanish_decks($this->number_of_decks);
	...
	function load_tournament_action(){
		//check if BJ, get a random team, store data, return logo	
		$logo = "";
		if($this->game_status == "player_blackjack" || $this->game_status == "push_blackjack"){
		
			$contest = get_active_contest_by_player($this ->player);
			
			if(!is_null($contest)){
		
				//put A in top card
				$new_cards = array();
				if(contains($this->player_hand[0],"A")){
					$new_cards[0] = $this->player_hand[1];
					$new_cards[1] = $this->player_hand[0];
				}else{
					$new_cards = $this->player_hand;	
				}
				$this->player_hand = $new_cards;
				
				$team = get_random_contest_team($contest ->vars["id"]);
				
				if(!is_null($team)){
								
					$record = new _contest_record();
					$record ->vars["contest"] = $contest ->vars["id"];
					$record ->vars["team"] = $team ->vars["id"];
					$record ->vars["player"] = $this ->player;
					$record ->vars["tdate"] = date("Y-m-d H:i:s");
					$record ->vars["points"] = $team ->vars["points"];
					$record->insert();
					
					$logo = $contest ->vars["imgs_folder"]."/".$team ->vars["image"];
	...
	function load_tournament_action_hand2(){
		//check if BJ, get a random team, store data, return logo	
		$logo = "";
	...
	function grade_game(){
		...
		}else if($player_total == $dealer_total && $player_total == 21){ // 21 push, player wins
		...
		//bonus hands:
		$cards_count = count($this ->player_hand);
		if($player_total == 21 && $cards_count > 2){
			
			if($cards_count == 3){
				
				$price_678 = $this->get_678_price_value();
				if($price_678 > 0){
					$this->win_amount = $this->bet_amount * $price_678;	
					$this->settle = $this->win_amount;	
				}else{
					
					$price_777 = $this->get_777_price_value();
					if($price_777 > 0){
						$this->win_amount = round($this->bet_amount * $price_777,1);	
						$this->settle = $this->win_amount;	
					}
				}
				...
				$this->settle = $this->win_amount;	
			}else if($cards_count == 6){
				$this->win_amount = $this->bet_amount * 2;	
				$this->settle = $this->win_amount;
			}else if($cards_count >= 7){
				$this->win_amount = $this->bet_amount * 3;	
				$this->settle = $this->win_amount;
			}
	...
	function get_777_price_value(){
		$price = 0;
		$values = array();
		$symbols = array();
		
		foreach($this ->player_hand as $card){		
			$values[] = $this ->get_card_value($card);
			$symbols[] = $this ->get_card_color($card);			
		}		
		
		if($values[0] == 7 && $values[1] == 7 && $values[2] == 7){
			
			if($symbols[0] == "S" && $symbols[1] == "S" && $symbols[2] == "S"){
				$price = 3;
				$suited = true;
			...
			//SUPERBONUS
			if($suited && $this->get_card_value($this ->dealer_hand[1]) == 7 ){
				
				if($this->bet_amount >= 25){
					$price = round(5000/$this->bet_amount,5);
				}else{
					$price = round(1000/$this->bet_amount,5);
				}
	...
	function get_678_price_value(){
		...
		if(in_array(6,$values) && in_array(7,$values) && in_array(8,$values)){
	...
	function double(){
		...
		$this->session ->vars["settle"] = ($this->session ->vars["settle"] ?? 0) + $this->settle;
		$this->session->update("bet_amount","settle");
		return $this->hit();
	...
	function deal(){
		...
		if($this ->player == 38 || $this ->player == 628){ //betowites, SBO5000 player for testing
		...
		$session ->vars["bj_type"] = "spanish";
		
		$player_total = $this->get_hand_value("player", true);	
		$dealer_total = $this->get_hand_value("dealer", true);	
		
		if($this ->is_blackjack($this ->player_hand[0],$this ->player_hand[1])){ //player blackjack always win
			$this->game_status = "player_blackjack";
			$this->game_finished = 1;
