...
class _holdem_poker{
	...
	function deal($bet_amount){
		...
		$session ->vars["poker_type"] = "holdem";
	...
	function call(){
		...
		$table_cards[] = $this->deal_card();
		$table_cards[] = $this->deal_card();
		$table_cards[] = $this->deal_card();
		$table_cards_str = implode(",",$table_cards);
		
		$all_cards = array_merge($table_cards,explode(",",$this->session ->vars["player_hand"]));
		$this->player_hand_level = $this->calculate_hand_level($all_cards);
		
		$this->session ->vars["call_bet"] = ($this->session ->vars["ante_bet"]*2);
		$this->session ->vars["table_cards"] = $table_cards_str;
		$this->session ->vars["status"] = "called";
	...
	function turn($bet){
		...
		$table_card = $this->deal_card();
		
		if($bet){
			$turn_bet = $this->session ->vars["ante_bet"];		
		}else{
			$turn_bet = 0;
		}
		
		$all_cards = array_merge(array($table_card),explode(",",$this->session ->vars["table_cards"]),explode(",",$this->session ->vars["player_hand"]));
		$this->player_hand_level = $this->get_best_hand_level($all_cards);
		
		$this->session ->vars["turn_bet"] = $turn_bet;
		$this->session ->vars["table_cards"] .= ",".$table_card;
		$this->session ->vars["status"] = "turned";
		$this->session ->vars["settle"] += ($this->session ->vars["turn_bet"]*-1);
		$this->session ->update();	
	...
	function river($bet){
		...
		if($bet){
			$river_bet = $this->session ->vars["ante_bet"];		
		}else{
			$river_bet = 0;
		}
		
		$pall_cards = array_merge(array($table_card),explode(",",$this->session ->vars["table_cards"]),explode(",",$this->session ->vars["player_hand"]));
		$this->player_hand_level = $this->get_best_hand_level($pall_cards);
		
		$dall_cards = array_merge(array($table_card),explode(",",$this->session ->vars["table_cards"]),explode(",",$this->session ->vars["dealer_hand"]));
		$this->dealer_hand_level = $this->get_best_hand_level($dall_cards);
		
		$this->session ->vars["river_bet"] = $river_bet;
		$this->session ->vars["table_cards"] .= ",".$table_card;
		$this->session ->vars["settle"] += ($this->session ->vars["river_bet"]*-1);
		$this->session ->update();	
	...
	function grade(){
		...
		if($this->player_hand_level > $this->dealer_hand_level || $tie_winner == "player"){
			...
			$this->winner = "player";
			$this->win_amount = 0;
			$this->win_amount += $this->session ->vars["call_bet"];
			$this->win_amount += $this->session ->vars["turn_bet"];
			$this->win_amount += $this->session ->vars["river_bet"];
			if($this->player_hand_level >= 4){ //straight or better
				$this->win_amount += $this->session ->vars["ante_bet"];
			}
			
			$this->credit_amount = $this->win_amount;
			$this->credit_amount += ($this->session ->vars["call_bet"]+$this->session ->vars["turn_bet"]+$this->session ->vars["river_bet"]+$this->session ->vars["ante_bet"]);
	...
	function calculate_hand_level($hand){
