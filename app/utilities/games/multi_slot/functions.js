//Set game vars
var core_url = "/utilities/games/multi_slot/action.php?gid=" + gid + "&";
...
function preload() {
  ...
  this.load.image(
    "pay_table",
    "utilities/games/multi_slot/imgs/pay_table.jpg?v=12",
  );
  ...
  this.load.image(
    "figure_beetle",
    "utilities/games/multi_slot/imgs/figures/beetle.png",
  );
  this.load.image(
    "figure_cross",
    "utilities/games/multi_slot/imgs/figures/cross.png",
  );
  this.load.image(
    "figure_eye",
    "utilities/games/multi_slot/imgs/figures/eye.png",
  );
  this.load.image(
    "figure_pyramid",
    "utilities/games/multi_slot/imgs/figures/pyramid.png",
  );
  this.load.image(
    "figure_tuta",
    "utilities/games/multi_slot/imgs/figures/tuta.png",
  );
  this.load.image(
    "figure_vessel",
    "utilities/games/multi_slot/imgs/figures/vessel.png",
  );
  this.load.image(
    "figure_J",
    "utilities/games/multi_slot/imgs/figures/J.png?v1",
  );
  this.load.image(
    "figure_Q",
    "utilities/games/multi_slot/imgs/figures/Q.png?v2",
  );
  this.load.image(
    "figure_K",
    "utilities/games/multi_slot/imgs/figures/K.png?v1",
  );
  this.load.image(
    "figure_A",
    "utilities/games/multi_slot/imgs/figures/A.png?v1",
  );

  this.load.image(
    "line_screen",
    "utilities/games/multi_slot/imgs/line_screen.png?v1",
  );
  for (var i = 1; i <= 25; i++) {
    this.load.image(
      "line_" + i,
      "utilities/games/multi_slot/imgs/lines/" + i + ".png",
    );
  }
  ...
  for (var i = 0; i <= 72; i += 2) {
    this.load.image(
      "animation_" + i,
      "utilities/games/multi_slot/imgs/animation1/Egypt-Animation" + i + ".png",
    );
  }
  ...
  this.load.audio(
    "spin",
    "utilities/games/multi_slot/sounds/spin_sound.wav?v3",
  );
...
function create() {
  ...
  spinning_sound = this.sound.add("spin", {
    mute: false,
    volume: 1,
    rate: 1,
    detune: 0,
  ...
  win_music = this.sound.add("big_win", {
    mute: false,
    volume: 1,
    rate: 1,
    detune: 0,
  ...
  var xfposes = [13, 3.7, 2.26, 1.62, 1.25];

  var fi = 0;
  var fignames = new Array(
    "beetle",
    "cross",
    "eye",
    "pyramid",
  ...
  for (var i = 0; i < xfposes.length; i++) {
    for (var n = 0; n < yfposes.length; n++) {
      figures[fi] = this.add.image(
        game_width / xfposes[i],
        game_height / yfposes[n],
        "figure_" + fignames[Math.floor(Math.random() * 10)],
      );
  ...
  /*var back_top = this.add.image(game_width/2, 0, 'back_top');
	back_top.displayWidth=game.config.width; 
	back_top.scaleY=back_top.scaleX;
	back_top.y = back_top.displayHeight/2;*/

  var back_down = this.add.image(game_width / 2, 0, "back_down");
  back_down.displayWidth = game.config.width;
  back_down.scaleY = back_down.scaleX;
  back_down.y = game_height - back_down.displayHeight / 2;

  var screen_lines_positions = [
    [2.3, 2.49],
    [2.3, 6.43],
    [2.3, 1.55],
    [2.87, 1.9],
  ...
  for (var i = 0; i < 25; i++) {
    lines.push(
      this.add.image(game_width / 2.3, game_height / 2.5, "line_" + (i + 1)),
    );
  ...
  //set text settings
  line_text_style = {
    font: game_width * 0.018 + "px Arial",
    fill: "#2aff00",
    align: "left",
  };
  ...
  for (var i = 0; i < 25; i++) {
    lines_screens.push(
      this.add.image(
        game_width / screen_lines_positions[i][0],
        game_height / screen_lines_positions[i][1],
        "line_screen",
      ),
    );
  ...
  //set texts positions
  bet_amount_text = this.add.text(
    game_width / 10.5,
    game_height / 1.5,
    current_bet,
    machine_text_style,
  );
  lines_amount_text = this.add.text(
    game_width / 3.9,
    game_height / 1.5,
    25,
    machine_text_style,
  );
  balance_amount_text = this.add.text(
    game_width / 1.09,
    game_height / 15.5,
    current_balance,
    machine_text_style,
  );
  total_amount_text = this.add.text(
    game_width / 1.09,
    game_height / 4.5,
    current_bet * max_lines,
    machine_text_style,
  );
  win_amount_text = this.add.text(
    game_width / 1.08,
    game_height / 2.3,
    "0",
    machine_text_style,
  ...
  info_btn = this.add
    .image(game_width / 1.06, game_height / 9.76, "info")
    .setInteractive();
  ...
  pay_table = this.add
    .image(game_width / 2, game_height / 2, "pay_table")
    .setInteractive();
  pay_table.displayWidth = game.config.width * 0.9;
  ...
  $("#betmax_btn").css("margin-left", game_width * 0.02 + "px");
  $("#spin_btn").css("margin-left", game_width * 0.31 + "px");
...
function update_win_amount(amount) {
  ...
  if (win_x5) {
    ...
    win_music.play();
    setTimeout("win_animation_fadeout = true;", 5000);
  }
...
function update_balance(new_balance) {
  ...
  balance_amount_text.text =
    Math.round((current_balance + Number.EPSILON) * 100) / 100;
...
function show_winning_lines() {
  ...
  if (winning_lines[0] != "") {
    ...
    for (var i = 0; i < winning_lines.length; i++) {
      ...
      var win_line_amount = parts[1]; //not used yet, can be used to display line win amount
        ...
        function (data) {
          if (!data.error) {
            ...
            for (var i = 0; i < positions.length; i++) {
              figures[i].setTexture("figure_" + positions[i]);
            ...
            setTimeout(
              "update_win_amount(" + data.win_amount + ")",
              timer + 300,
            );
...
function change_bet(type) {
  if (
    ...
    if (current_bet * 1 + step <= max_amount) {
      current_bet =
        Math.round((current_bet * 1 + step + Number.EPSILON) * 100) / 100;
    } else {
      current_bet = min_amount;
    }
    update_screens();
