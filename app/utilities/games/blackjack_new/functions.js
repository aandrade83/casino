...
var core_url = 'https://play.casinogamesonline.com/utilities/games/blackjack_new/action.php?gid='+gid+'&';
...
function preload (){
	...
	//Load elements
	//Background
	this.load.image('background', 'utilities/games/blackjack_new/imgs/back.jpg');
	//chips
	...
	//Others
	this.load.image('arrow', 'utilities/games/blackjack_new/imgs/green_arrow.png');
	//sounds
	this.load.audio('card_flip_fx', 'utilities/games/blackjack_new/sounds/cardflip.wav');
...
function update (){
	...
	for(var r = 0; r<creation_cards.length; r++){
		...
		moving_cards.push({posx:creation_cards[r].posx,posy:creation_cards[r].posy,image:creation_cards[r].image,object:new_card});
...
function deal_card(card,px,py){
	creation_cards.push({image:card, posx:px, posy:py});
	sound_flip.play();
}
	...
	if(game_status == "place_bets"){
		...
		$.getJSON(core_url + "action=deal&bet=" + current_bet  + "&pnr="+player_number,function(data){
			if(!data.error){ //change the way to check error 
				...
				setTimeout('deal_card("card_'+temp_player_cards[1]+'",player_cards_positions[1].x,player_cards_positions[1].y);',1000);
		...
		if(current_bet <= current_balance){
			...
			$.getJSON(core_url + "action=split" ,function(data){
				if(!data.error){ //change the way to check error
					...
					var temp_player_cards = data.player_hand.split(",");
					var temp_player_cards2 = data.player_hand2.split(",");
					setTimeout('deal_card("card_'+temp_player_cards2[1]+'",player2_cards_positions[1].x,player2_cards_positions[1].y);',1000);
